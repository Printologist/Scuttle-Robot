...
BAUD_RATE = 115200
OBSTACLE_DISTANCE_MM = 305  # stop if anything closer than 1 foot
...
class OpenAIVoiceAssistant:
    ...
    def __init__(
        ...
        self.system_prompt = (
            ...
            "When the user asks you to move or dance, respond with ONLY one of these exact words: "
            "MOVE_FORWARD, MOVE_BACKWARD, TURN_LEFT, TURN_RIGHT, STOP, DANCE. ")
        self.chat_history = []
    ...
    async def get_response(self, user_text: str) -> str:
        ...
        try:
            ...
            movement_commands = ["MOVE_FORWARD", "MOVE_BACKWARD", "TURN_LEFT", "TURN_RIGHT", "STOP", "DANCE"]
            if assistant_message.strip().upper() in movement_commands:
                ...
                if assistant_message.strip().upper() == "DANCE":
...
async def main():
    ...
    try:
        assistant = OpenAIVoiceAssistant(robot, "wake-word", "speaker", "camera", "base", "Lidar", ser)
